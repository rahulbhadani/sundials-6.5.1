...
class CMakeBuild(build_ext):
    def build_extension(self, ext):
        ...
        print(' '.join(['cmake', '--build', '.', '--target', 'sundialspy'] + build_args))
        subprocess.check_call(
            ['cmake', '--build', '.', '--target', 'sundialspy'] + build_args, cwd=self.build_temp
        )

setup(
    packages=find_packages('python/src'),
    package_dir={"sundials": "python/src/sundials"},
    python_requires=">=3.6",
    setup_requires=[
        'pytest-runner',
        'wheel'
